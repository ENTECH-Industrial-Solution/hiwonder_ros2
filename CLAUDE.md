...
- **Pick and place (`pick_place.launch.py`).** Not MoveIt: `robot_moveit_config` sets `position_only_ik: true`, which can't constrain grasp orientation, so the pick path uses its own closed-form IK instead. Grasping is a `DetachableJoint` weld on `link5`, not friction — and Gazebo forms that weld the instant the cube spawns, before any node commands it, so the node must publish a detach at startup or the first arm move drags the whole scene. The pedestal and cubes are spawned at run time rather than added to `rospider_room.sdf`, so that world file keeps backing `maps/rospider_room.*` and Nav2's AMCL initial pose. The three drop slots sit in a row, not stacked: at stacking height the arm has no steep-approach solution left, and the only shallow ones sweep the held cube through the cubes already placed. `~/start` runs the whole cycle; `~/pick` stops in a `CARRY` state and holds until `~/place`, so the robot can be driven between the two. `CARRY` is exempt from the state timeout for that reason, and every target point (`carry_point`, `place_point`, `drop_slots`, and a coordinate passed to `~/place`) is in `base_footprint`, so it travels with the robot and no world-frame bookkeeping is needed.
- **YOLO tuner (`yolo_detect.py`).** `tune:=true` opens a two-control Tk window (confidence slider, one checkbox per model class) built on `tkview.py`; `conf`/`classes` are saved to `~/.ros/yolo_detect_tuned.json` with the same YAML→JSON precedence as the other tuners (`rospider_gazebo/yolo_settings.py`, tested). Empty `classes` means no filter, and `yaml_block` comments the key out in that case because an empty YAML list cannot be declared as a ROS 2 parameter.
- **AprilTag behaviours (`apriltag_detect.py`).** `tune:=true` opens a Tk window (workshop-simple by default, an `Advanced` toggle reveals the aruco `DetectorParameters` sliders and the 9 `control` gains; `rospider_gazebo/tkview.py` holds the shared Tk helpers) for assigning each tag id an action (`none/approach/place/stop`) and a `standoff`. Settings split `rospider_gazebo/tag_settings.py` (defaults, YAML/JSON merge, YAML rendering — `config/apriltag.yaml` overlaid key-by-key by `~/.ros/apriltag_tuned.json`, same precedence as `color_detect`; `behaviors_enabled` is never saved, so every launch starts with behaviours off) from `rospider_gazebo/tag_behavior.py` (pure Python, tested: nearest actionable tag wins, `stop` beats all, turns in place before driving when the bearing exceeds `turn_first_rad`, `place` fires `/pick_and_place/place` once per sighting, a lost tag holds zero for `lost_timeout` then releases `/controller/cmd_vel`). The node steers in a level frame at `base_footprint`, not the camera's — the camera rides the wrist (`link4`) and points down/sideways in `CARRY`, where camera x/z say nothing about heading — so every tag pose (real or remembered) is mapped through TF before it reaches `tag_behavior`. Because the held cube blocks the wrist camera during `CARRY`, the node also remembers every tag pose in `memory_frame` (default `odom`) and feeds `approach`/`place` a virtual sighting when the live one is gone. `scene:=false` on `pick_place.launch.py` skips the pick pedestal/cubes for a tag-only demo, since the default spawn pose sits close enough to ram the pedestal on any forward move. `behaviors_enabled`, per-tag actions/standoffs, `control.*` and `detector.*` are all also live-settable with plain `ros2 param set` (the GUI doesn't read them back).
